...
{
	public EntityChocolateCreamBall(World world)
	{
		super(world);
	}

	public EntityChocolateCreamBall(World world, EntityLivingBase living)
	{
		super(world, living);
	}
	...
	protected void onImpact(MovingObjectPosition moving)
	{
		if (moving.entityHit != null)
		{
			...
			{
				((EntityLivingBase)moving.entityHit).addPotionEffect(new PotionEffect(Potion.blindness.id, 100, 2));
		...
		for (int i = 0; i < 8; ++i)
		{
			MorePlanetsCore.proxy.spawnParticle(ParticleTypesMP.CHOCOLATE_CREAM_BALL, this.posX, this.posY, this.posZ);
		}
