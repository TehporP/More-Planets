...
public class ItemBlockStrawberryCreamLayer extends ItemBlockMorePlanets
{
	...
	{
		IBlockState state = world.getBlockState(pos);
		return state.getBlock() != FronosBlocks.strawberry_cream_layer || (Integer)state.getValue(BlockCreamLayer.LAYERS) > 7 ? super.canPlaceBlockOnSide(world, pos, side, player, stack) : true;
	}
}