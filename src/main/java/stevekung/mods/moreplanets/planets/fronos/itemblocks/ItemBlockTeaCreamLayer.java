...
public class ItemBlockTeaCreamLayer extends ItemBlockMorePlanets
{
	...
	{
		IBlockState state = world.getBlockState(pos);
		return state.getBlock() != FronosBlocks.tea_cream_layer || (Integer)state.getValue(BlockCreamLayer.LAYERS) > 7 ? super.canPlaceBlockOnSide(world, pos, side, player, stack) : true;
	}
}