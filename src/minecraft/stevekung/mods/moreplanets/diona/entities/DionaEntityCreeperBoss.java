...
public class DionaEntityCreeperBoss extends EntityMob implements IEntityBreathable, IBossDisplayData, IRangedAttackMob, IBoss
{
	...
	public DionaEntityCreeperBoss(World par1World)
	{
		super(par1World);
	...
	protected void applyEntityAttributes()
	{
		...
		this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setAttribute(250.0F * GCCoreConfigManager.dungeonBossHealthMod);
		this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).setAttribute(0.05F);
	}

	public DionaEntityCreeperBoss(World world, Vector3 vec)
	{
		this(world);
	...
	protected void onDeathUpdate()
	{
		...
		if (this.deathTicks >= 180 && this.deathTicks <= 200)
		{
			...
			DionaEntityDelriumWorm worm = new DionaEntityDelriumWorm(this.worldObj);
		...
		if (this.deathTicks == 200 && !this.worldObj.isRemote)
		{
			...
			{
				if (tile instanceof DionaTileEntityTreasureChest)
				{
					...
					{
						if (!((DionaTileEntityTreasureChest) tile).locked)
						{
							((DionaTileEntityTreasureChest) tile).locked = true;
						}

						for (int k = 0; k < ((DionaTileEntityTreasureChest) tile).getSizeInventory(); k++)
						{
							((DionaTileEntityTreasureChest) tile).setInventorySlotContents(k, null);
						}

						ChestGenHooks info = ChestGenHooks.getInfo(ChestGenHooks.DUNGEON_CHEST);

						WeightedRandomChestContent.generateChestContents(this.rand, info.getItems(this.rand), (DionaTileEntityTreasureChest) tile, info.getCount(this.rand));
						WeightedRandomChestContent.generateChestContents(this.rand, info.getItems(this.rand), (DionaTileEntityTreasureChest) tile, info.getCount(this.rand));

						((DionaTileEntityTreasureChest) tile).setInventorySlotContents(this.rand.nextInt(((DionaTileEntityTreasureChest) tile).getSizeInventory()), this.getGuaranteedLoot(this.rand));
			...
			this.entityDropItem(new ItemStack(DionaItems.dionaKeyTreasure.itemID, 1, 0), 0.5F);
	...
	private void func_82209_a(int par1, double par2, double par4, double par6)
	{
		...
		double d7 = par4 - d4;
		double d8 = par6 - d5;
		DionaEntityProjectileFronisiumTNT parasite = new DionaEntityProjectileFronisiumTNT(this.worldObj, this, d6 * 0.5D, d7 * 0.5D, d8 * 0.5D);

		parasite.posY = d4;
		parasite.posX = d3;
		parasite.posZ = d5;
		this.worldObj.spawnEntityInWorld(parasite);
	}
