...
public class DionaItemSpaceshipTier3 extends Item implements IHoldableItem
{
	public DionaItemSpaceshipTier3(int par1)
	{
		super(par1);
		this.setMaxDamage(0);
	...
	public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10)
	{
		...
		else
		{
			...
			if (amountOfCorrectBlocks == 9)
			{
				...
				{
					rocket = new DionaEntityRocketT3(par3World, centerX, centerY + 4.2D, centerZ, EnumRocketType.values()[par1ItemStack.getItemDamage()]);
				}
				else
				{
					rocket = new DionaEntityRocketT3NoFlag(par3World, centerX, centerY + 4.2D, centerZ, EnumRocketType.values()[par1ItemStack.getItemDamage() - 10]);
				}
				...
				{
					if (rocket instanceof DionaEntityRocketT3)
					{
						((DionaEntityRocketT3) rocket).fuelTank.fill(new FluidStack(GalacticraftCore.fluidFuel, rocket.getMaxFuel()), true);
					}
					else if (rocket instanceof DionaEntityRocketT3NoFlag)
					{
						((DionaEntityRocketT3NoFlag) rocket).fuelTank.fill(new FluidStack(GalacticraftCore.fluidFuel, rocket.getMaxFuel()), true);
					}
				}
			}
			else
