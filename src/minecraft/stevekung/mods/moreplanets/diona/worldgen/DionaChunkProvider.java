...
import stevekung.mods.moreplanets.diona.worldgen.dungeon.DionaRoomBoss;
import stevekung.mods.moreplanets.diona.worldgen.dungeon.DionaRoomChests;
import stevekung.mods.moreplanets.diona.worldgen.dungeon.DionaRoomEmpty;
import stevekung.mods.moreplanets.diona.worldgen.dungeon.DionaRoomSpawner;
import stevekung.mods.moreplanets.diona.worldgen.dungeon.DionaRoomTreasure;
import stevekung.mods.moreplanets.diona.worldgen.pit.DionaMapGenCreeperNest;

public class DionaChunkProvider extends ChunkProviderGenerate
{
	final short topBlockID = (short) DionaBlocks.dionaBasicBlock.blockID;
	final byte topBlockMeta = 6;
	final short fillBlockID = (short) DionaBlocks.dionaBasicBlock.blockID;
	final byte fillBlockMeta = 7;
	final short lowerBlockID = (short) DionaBlocks.dionaBasicBlock.blockID;
	final byte lowerBlockMeta = 8;
	...
	public DionaBiomeDecorator biomedecoratorplanet = new DionaBiomeDecorator(DionaBiomeGenBase.diona);

	private final World worldObj;
	private final DionaMapGenCreeperNest delriumWormNest = new DionaMapGenCreeperNest();

	private final GCCoreMapGenDungeon dungeonGenerator = new GCCoreMapGenDungeon(DionaBlocks.dionaBasicBlock.blockID, 14, 0, 16, 3);

	{
		this.dungeonGenerator.otherRooms.add(new DionaRoomEmpty(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new DionaRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new DionaRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new DionaRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new DionaRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new DionaRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		...
		this.dungeonGenerator.otherRooms.add(new DionaRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new DionaRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new DionaRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new DionaRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.bossRooms.add(new DionaRoomBoss(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.treasureRooms.add(new DionaRoomTreasure(null, 0, 0, 0, ForgeDirection.UNKNOWN));
	}

	private BiomeGenBase[] biomesForGeneration = { DionaBiomeGenBase.diona };

	private final DionaGenCaves caveGenerator = new DionaGenCaves();
	private final DionaCavern caveGenerator2 = new DionaCavern();
	...
	public DionaChunkProvider(World par1World, long par2, boolean par4)
	{
	...
	public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
	{
		...
		for (int x = 0; x < DionaChunkProvider.CHUNK_SIZE_X; x++)
		{
			for (int z = 0; z < DionaChunkProvider.CHUNK_SIZE_Z; z++)
			{
				...
				for (int y = 0; y < DionaChunkProvider.CHUNK_SIZE_Y; y++)
				{
					if (y < DionaChunkProvider.MID_HEIGHT + yDev)
					{
	...
	public Chunk provideChunk(int par1, int par2)
	{
		...
		int[] var3 = null;
		this.delriumWormNest.generate(this, this.worldObj, par1, par2, var3);
		...
		if (!var4.isTerrainPopulated && DionaConfigManager.generateOtherMods)
		{
		...
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
		{
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
			{
				for (int x = 0; x < DionaChunkProvider.CHUNK_SIZE_X; x++)
				{
					for (int z = 0; z < DionaChunkProvider.CHUNK_SIZE_Z; z++)
					{
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * DionaChunkProvider.CHUNK_SIZE_X + x, cz * DionaChunkProvider.CHUNK_SIZE_Z + z) / DionaChunkProvider.CRATER_PROB)
						{
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
	{
		...
		this.delriumWormNest.generateStructuresInChunk(this.worldObj, new Random(), par2, par3);
	...
	{
		return DionaConfigManager.generateOtherMods ? "RandomLevelSource" : "DionaLevelSource";
	}
	...
	public List getPossibleCreatures(EnumCreatureType par1EnumCreatureType, int i, int j, int k)
	{
		if (par1EnumCreatureType == EnumCreatureType.monster)
		{
			...
			monsters.add(new SpawnListEntry(DionaEntityMinionCreeper.class, 8, 2, 3));
		...
		else if (par1EnumCreatureType == EnumCreatureType.creature)
		{
			...
			creatures.add(new SpawnListEntry(DionaEntitySpaceWolf.class, 1, 1, 1));
			return creatures;
		}
		else
		{
