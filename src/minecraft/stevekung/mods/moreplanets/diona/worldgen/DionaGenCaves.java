...
public class DionaGenCaves extends GCCoreMapGenBaseMeta
{
							...
							for (int localX = l1; localX < i2; localX++)
							{
								...
								for (int localZ = l2; localZ < i3; localZ++)
								{
									...
									if (xfactorSq + zfactorSq < 1.0D)
									{
										...
										{
											if (blockIdArray[coords] == DionaBlocks.dionaBasicBlock.blockID)
											{//Dirt-Stone
												if (metaArray[coords] == 7 || metaArray[coords] == 8)
												{
													blockIdArray[coords] = 0;
												}//Top of Block
												else if (metaArray[coords] == 6 && random.nextInt(DionaGenCaves.BREAK_THROUGH_CHANCE) == 0)
												{
													blockIdArray[coords] = 0;
												}
											}
