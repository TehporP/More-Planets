...
public class DionaRoomBoss extends GCCoreDungeonRoom
{
	public int sizeX;
	public int sizeY;
	public int sizeZ;
	...
	public DionaRoomBoss(GCCoreMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
		...
		for (int i = this.posX - 1; i <= this.posX + this.sizeX; i++)
		{
			for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
			{
				for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
				{
					...
					{
						this.placeBlock(chunk, meta, i, j, k, cx, cz, DionaBlocks.minionCreeperEgg.blockID, 0);
					}
					else
					{
	...
	{
		return new DionaRoomBoss(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		...
		this.worldObj.setBlock(this.spawnerCoords.posX, this.spawnerCoords.posY, this.spawnerCoords.posZ, DionaBlocks.dionaBasicBlock.blockID, 15, 3);
		...
		if (tile == null || !(tile instanceof DionaTileEntityDungeonSpawner))
		{
			GCCoreTileEntityDungeonSpawner spawner = new DionaTileEntityDungeonSpawner();
