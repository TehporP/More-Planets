...
public class DionaRoomChests extends GCCoreDungeonRoom
{
	int sizeX;
	int sizeY;
	int sizeZ;
	...
	public DionaRoomChests(GCCoreMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
	...
	public void generate(short[] chunk, byte[] meta, int cx, int cz)
	{
		...
		if (this.placeBlock(chunk, meta, hx, this.posY, hz, cx, cz, DionaBlocks.dionaAncientChest.blockID, 0))
		{
	...
	{
		return new DionaRoomChests(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		...
		{
			this.worldObj.setBlock(this.chests.get(0).posX, this.chests.get(0).posY, this.chests.get(0).posZ, DionaBlocks.dionaAncientChest.blockID, 0, 2);
			DionaTileEntityAncientChest chest = (DionaTileEntityAncientChest) this.worldObj.getBlockTileEntity(this.chests.get(0).posX, this.chests.get(0).posY, this.chests.get(0).posZ);
