...
package stevekung.mods.moreplanets.diona.worldgen.dungeon;
...
public class DionaRoomTreasure extends GCCoreDungeonRoom
{
	int sizeX;
	int sizeY;
	int sizeZ;
	...
	public DionaRoomTreasure(GCCoreMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
	...
	public void generate(short[] chunk, byte[] meta, int cx, int cz)
	{
		...
		if (this.placeBlock(chunk, meta, hx, this.posY, hz, cx, cz, DionaBlocks.dionaTreasureChest.blockID, 0))
		{
	...
	public ItemStack getGuaranteedLoot(Random rand)
	{
		...
		{
		case 0:
			return new ItemStack(PolongniusItems.schematicT4, 1, 0);
		case 1:
			return new ItemStack(DionaBlocks.fronisiumTnt, 4, 0);
		}
		return null;
	}
	...
	protected void handleTileEntities(Random rand)
	{
		for (final ChunkCoordinates chestCoords : this.chests)
		{
			...
			if (chest != null && chest instanceof DionaTileEntityTreasureChest)
			{
				ChestGenHooks info = ChestGenHooks.getInfo(ChestGenHooks.DUNGEON_CHEST);

				WeightedRandomChestContent.generateChestContents(rand, info.getItems(rand), (DionaTileEntityTreasureChest) chest, info.getCount(rand));

				((DionaTileEntityTreasureChest) chest).setInventorySlotContents(rand.nextInt(((DionaTileEntityTreasureChest) chest).getSizeInventory()), this.getGuaranteedLoot(rand));
			}
		}
	}
	...
	{
		return new DionaRoomTreasure(dungeon, x, y, z, dir);
	}
}