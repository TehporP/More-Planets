...
public class DionaComponentCreeperPitRoom extends GCCoreStructureComponent
{
	...
	public DionaComponentCreeperPitRoom(int type, World world, Random par2Random, int x, int y, int z, int height, int width, int cbm)
	{
		super(type);
		...
		for (int y = 0; y < this.height; y++)
		{
			for (int x = 0; x < 7; x++)
			{
				for (int z = 0; z < 7; z++)
				{
					...
					{
						this.placeBlockAtCurrentPosition(world, DionaBlocks.dionaBasicBlock.blockID, 14, x, y, z, this.getBoundingBox());
					}
					else
					{
		...
		for (int y = this.height - 1; y > 10; y--)
		{
			for (int x = 0; x < this.width; x++)
			{
				for (int z = 0; z < this.width; z++)
				{
					...
					{
						if (world.getBlockId(this.getBoundingBox().minX + x, this.getBoundingBox().minY + y, this.getBoundingBox().minZ + z) == DionaBlocks.dionaBasicBlock.blockID && world.getBlockMetadata(x, y, z) == 14) {
							;
						}
						{
							for (int i = -2; i < 2; i++)
							{
								for (int j = -2; j < 2; j++)
								{
									...
									{
										this.placeBlockAtCurrentPosition(world, DionaBlocks.dionaBasicBlock.blockID, 14, x + i, y, z + j, this.getBoundingBox());
									}
									...
									if (rand.nextInt(5) == 0 && world.getBlockId(this.getBoundingBox().minX + x + i, this.getBoundingBox().minY + y + 1, this.getBoundingBox().minZ + z + j) == 0 && world.getBlockId(this.getBoundingBox().minX + x + i, this.getBoundingBox().minY + y, this.getBoundingBox().minZ + z + j) == DionaBlocks.dionaBasicBlock.blockID && world.getBlockMetadata(x, y, z) == 14)
									{
										this.placeBlockAtCurrentPosition(world, DionaBlocks.minionCreeperEgg.blockID, 0, x + i, y + 1, z + j, this.getBoundingBox());
									}
								}
							}
							...
							if (rand.nextInt(7) == 0)
							{
								if (x > 0 && x < 7 && z > 0 && z < 7)
								{
									...
									{
										this.placeBlockAtCurrentPosition(world, DionaBlocks.minionCreeperEgg.blockID, 0, x, y + 2, z, this.getBoundingBox());
									}
