...
public class FronosEntityJellyStrawberrySlime extends EntityLiving implements IMob, IEntityBreathable
{
	...
	public FronosEntityJellyStrawberrySlime(World par1World)
	{
		super(par1World);
	...
	public void onUpdate()
	{
		...
		if (this.onGround && !flag)
		{
			...
			for (int j = 0; j < i * 8; ++j)
			{
				...
				MorePlanetCore.proxy.spawnParticle("jellyStrawberry", this.posX + f2, this.boundingBox.minY, this.posZ + f3);
			}
	...
	protected FronosEntityJellyStrawberrySlime createInstance()
	{
		return new FronosEntityJellyStrawberrySlime(this.worldObj);
	}
	...
	public void setDead()
	{
		...
		if (!this.worldObj.isRemote && i > 1 && this.getHealth() <= 0.0F)
		{
			...
			for (int k = 0; k < j; ++k)
			{
				...
				FronosEntityJellyStrawberrySlime entityslime = this.createInstance();
	...
	protected void dropFewItems(boolean par1, int par2)
	{
		if (this.getSlimeSize() == 1)
		{
			...
			for (int k = 0; k < j; ++k)
			{
				this.entityDropItem(new ItemStack(FronosItems.jelly, 1, 2), 0.0F);
			}
		}
	}
