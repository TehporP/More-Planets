...
public class KapteynBChunkProvider extends ChunkProviderGenerate
{
	final short topBlockID = (short) KapteynBBlocks.kapteynBBasicBlock.blockID;
	final byte topBlockMeta = 0;
	final short fillBlockID = (short) KapteynBBlocks.kapteynBBasicBlock.blockID;
	final byte fillBlockMeta = 1;
	final short lowerBlockID = (short) KapteynBBlocks.kapteynBBasicBlock.blockID;
	...
	public KapteynBBiomeDecorator biomedecoratorplanet = new KapteynBBiomeDecorator(KapteynBBiomeGenBase.kapteynB);

	private final World worldObj;

	private final GCCoreMapGenDungeon dungeonGenerator = new GCCoreMapGenDungeon(KapteynBBlocks.kapteynBBasicBlock.blockID, 3, 8, 16, 3);
	...
	private BiomeGenBase[] biomesForGeneration = { KapteynBBiomeGenBase.kapteynB };

	private final GCCoreMapGenBaseMeta caveGenerator = new KapteynBGenCaves();
	...
	public KapteynBChunkProvider(World par1World, long par2, boolean par4)
	{
	...
	public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
	{
		...
		for (int x = 0; x < KapteynBChunkProvider.CHUNK_SIZE_X; x++)
		{
			for (int z = 0; z < KapteynBChunkProvider.CHUNK_SIZE_Z; z++)
			{
				...
				for (int y = 0; y < KapteynBChunkProvider.CHUNK_SIZE_Y; y++)
				{
					if (y < KapteynBChunkProvider.MID_HEIGHT + yDev)
					{
		...
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
		{
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
			{
				for (int x = 0; x < KapteynBChunkProvider.CHUNK_SIZE_X; x++)
				{
					for (int z = 0; z < KapteynBChunkProvider.CHUNK_SIZE_Z; z++)
					{
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * KapteynBChunkProvider.CHUNK_SIZE_X + x, cz * KapteynBChunkProvider.CHUNK_SIZE_Z + z) / KapteynBChunkProvider.CRATER_PROB)
						{
	...
	{
		return KapteynBConfigManager.generateOtherMods ? "RandomLevelSource" : "KapteynBLevelSource";
	}
