...
public class KapteynBGenCaves extends GCCoreMapGenBaseMeta
{
							...
							for (int localX = l1; localX < i2; localX++)
							{
								...
								for (int localZ = l2; localZ < i3; localZ++)
								{
									...
									if (xfactorSq + zfactorSq < 1.0D)
									{
										...
										{
											if (blockIdArray[coords] == KapteynBBlocks.kapteynBBasicBlock.blockID)
											{
												if (metaArray[coords] == 1 || metaArray[coords] == 2)
												{
													blockIdArray[coords] = 0;
												}
												else if (metaArray[coords] == 0 && random.nextInt(KapteynBGenCaves.BREAK_THROUGH_CHANCE) == 0)
												{
													blockIdArray[coords] = 0;
												}
											}
