...
import stevekung.mods.moreplanets.koentus.worldgen.dungeon.KoentusRoomBoss;
import stevekung.mods.moreplanets.koentus.worldgen.dungeon.KoentusRoomChests;
import stevekung.mods.moreplanets.koentus.worldgen.dungeon.KoentusRoomEmpty;
import stevekung.mods.moreplanets.koentus.worldgen.dungeon.KoentusRoomSpawner;
import stevekung.mods.moreplanets.koentus.worldgen.dungeon.KoentusRoomTreasure;
import stevekung.mods.moreplanets.koentus.worldgen.village.KoentusMapGenVillage;

public class KoentusChunkProvider extends ChunkProviderGenerate
{
	final short topBlockID = (short) KoentusBlocks.koentusBasicBlock.blockID;
	final byte topBlockMeta = 6;
	final short fillBlockID = (short) KoentusBlocks.koentusBasicBlock.blockID;
	final byte fillBlockMeta = 7;
	final short lowerBlockID = (short) KoentusBlocks.koentusBasicBlock.blockID;
	...
	public KoentusBiomeDecorator biomedecoratorplanet = new KoentusBiomeDecorator(KoentusBiomeGenBase.koentus);

	private final World worldObj;
	private final KoentusMapGenVillage villageGenerator = new KoentusMapGenVillage();

	private final GCCoreMapGenDungeon dungeonGenerator = new GCCoreMapGenDungeon(KoentusBlocks.koentusBasicBlock.blockID, 12, 8, 16, 3);

	{
		this.dungeonGenerator.otherRooms.add(new KoentusRoomEmpty(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new KoentusRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new KoentusRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new KoentusRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new KoentusRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new KoentusRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new KoentusRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new KoentusRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		...
		this.dungeonGenerator.otherRooms.add(new KoentusRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new KoentusRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.bossRooms.add(new KoentusRoomBoss(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.treasureRooms.add(new KoentusRoomTreasure(null, 0, 0, 0, ForgeDirection.UNKNOWN));
	}

	private BiomeGenBase[] biomesForGeneration = { KoentusBiomeGenBase.koentus };

	private final GCCoreMapGenBaseMeta caveGenerator = new KoentusGenCaves();
	...
	public KoentusChunkProvider(World par1World, long par2, boolean par4)
	{
	...
	public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
	{
		...
		for (int x = 0; x < KoentusChunkProvider.CHUNK_SIZE_X; x++)
		{
			for (int z = 0; z < KoentusChunkProvider.CHUNK_SIZE_Z; z++)
			{
				...
				for (int y = 0; y < KoentusChunkProvider.CHUNK_SIZE_Y; y++)
				{
					if (y < KoentusChunkProvider.MID_HEIGHT + yDev)
					{
	...
	public Chunk provideChunk(int par1, int par2)
	{
		...
		if (!var4.isTerrainPopulated && KoentusConfigManager.generateOtherMods)
		{
		...
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
		{
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
			{
				for (int x = 0; x < KoentusChunkProvider.CHUNK_SIZE_X; x++)
				{
					for (int z = 0; z < KoentusChunkProvider.CHUNK_SIZE_Z; z++)
					{
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * KoentusChunkProvider.CHUNK_SIZE_X + x, cz * KoentusChunkProvider.CHUNK_SIZE_Z + z) / KoentusChunkProvider.CRATER_PROB)
						{
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
	{
		...
		if (!KoentusConfigManager.disableKoentusVillageGen)
		{
	...
	{
		return KoentusConfigManager.generateOtherMods ? "RandomLevelSource" : "KoentusLevelSource";
	}
