...
public class KoentusGenCaves extends GCCoreMapGenBaseMeta
{
							...
							for (int localX = l1; localX < i2; localX++)
							{
								...
								for (int localZ = l2; localZ < i3; localZ++)
								{
									...
									if (xfactorSq + zfactorSq < 1.0D)
									{
										...
										{
											if (blockIdArray[coords] == KoentusBlocks.koentusBasicBlock.blockID)
											{
												if (metaArray[coords] == 7 || metaArray[coords] == 8)
												{
													blockIdArray[coords] = 0;
												}
												else if (metaArray[coords] == 6 && random.nextInt(KoentusGenCaves.BREAK_THROUGH_CHANCE) == 0)
												{
													blockIdArray[coords] = 0;
												}
											}
