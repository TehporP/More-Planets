...
public class KoentusRoomSpawner extends GCCoreDungeonRoom
{
	int sizeX;
	int sizeY;
	int sizeZ;
	...
	public KoentusRoomSpawner(GCCoreMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
	...
	{
		return new KoentusRoomSpawner(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		for (final ChunkCoordinates spawnerCoords : this.spawners)
		{
			if (this.worldObj.getBlockId(spawnerCoords.posX, spawnerCoords.posY, spawnerCoords.posZ) == Block.mobSpawner.blockID)
			{
				...
				if (spawner != null)
				{
					spawner.getSpawnerLogic().setMobID(KoentusRoomSpawner.getMob(rand));
				}
			}
		}
	}
