...
import stevekung.mods.moreplanets.nibiru.worldgen.dungeon.NibiruRoomBoss;
import stevekung.mods.moreplanets.nibiru.worldgen.dungeon.NibiruRoomChests;
import stevekung.mods.moreplanets.nibiru.worldgen.dungeon.NibiruRoomEmpty;
import stevekung.mods.moreplanets.nibiru.worldgen.dungeon.NibiruRoomSpawner;
import stevekung.mods.moreplanets.nibiru.worldgen.dungeon.NibiruRoomTreasure;

public class NibiruChunkProvider extends ChunkProviderGenerate
{
	final short topBlockID = (short) NibiruBlocks.nibiruBasicBlock.blockID;
	final byte topBlockMeta = 5;
	final short fillBlockID = (short) NibiruBlocks.nibiruBasicBlock.blockID;
	final byte fillBlockMeta = 6;
	final short lowerBlockID = (short) NibiruBlocks.nibiruBasicBlock.blockID;
	final byte lowerBlockMeta = 7;
	...
	public NibiruBiomeDecorator biomedecoratorplanet = new NibiruBiomeDecorator(NibiruBiomeGenBase.nibiru);

	private final World worldObj;

	private final NibiruGenCaves caveGenerator = new NibiruGenCaves();

	private final GCCoreMapGenDungeon dungeonGenerator = new GCCoreMapGenDungeon(NibiruBlocks.nibiruBasicBlock.blockID, 11, 8, 16, 6);

	{
		this.dungeonGenerator.otherRooms.add(new NibiruRoomEmpty(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new NibiruRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new NibiruRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new NibiruRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new NibiruRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new NibiruRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		...
		this.dungeonGenerator.otherRooms.add(new NibiruRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new NibiruRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new NibiruRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new NibiruRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.bossRooms.add(new NibiruRoomBoss(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.treasureRooms.add(new NibiruRoomTreasure(null, 0, 0, 0, ForgeDirection.UNKNOWN));
	}

	private BiomeGenBase[] biomesForGeneration = { NibiruBiomeGenBase.nibiru };
	...
	private static final double TERRAIN_HEIGHT_MOD = 45;
	private static final double SMALL_FEATURE_HEIGHT_MOD = 62;
	private static final double MOUNTAIN_HEIGHT_MOD = 168;
	private static final double VALLEY_HEIGHT_MOD = 112;
	...
	private static final double MAIN_FEATURE_FILTER_MOD = 12;
	private static final double LARGE_FEATURE_FILTER_MOD = 16;
	private static final double SMALL_FEATURE_FILTER_MOD = 16;

	public NibiruChunkProvider(World par1World, long par2, boolean par4)
	{
	...
	public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
	{
		...
		for (int x = 0; x < NibiruChunkProvider.CHUNK_SIZE_X; x++)
		{
			for (int z = 0; z < NibiruChunkProvider.CHUNK_SIZE_Z; z++)
			{
				final double baseHeight = this.noiseGen1.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * NibiruChunkProvider.TERRAIN_HEIGHT_MOD;
				final double smallHillHeight = this.noiseGen2.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * NibiruChunkProvider.SMALL_FEATURE_HEIGHT_MOD;
				...
				final double featureFilter = this.noiseGen5.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * NibiruChunkProvider.MAIN_FEATURE_FILTER_MOD;
				final double largeFilter = this.noiseGen6.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * NibiruChunkProvider.LARGE_FEATURE_FILTER_MOD;
				final double smallFilter = this.noiseGen7.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * NibiruChunkProvider.SMALL_FEATURE_FILTER_MOD - 0.5;

				mountainHeight = this.lerp(smallHillHeight, mountainHeight * NibiruChunkProvider.MOUNTAIN_HEIGHT_MOD, this.fade(this.clamp(mountainHeight * 2, 0, 1)));
				valleyHeight = this.lerp(smallHillHeight, valleyHeight * NibiruChunkProvider.VALLEY_HEIGHT_MOD - NibiruChunkProvider.VALLEY_HEIGHT_MOD + 9, this.fade(this.clamp((valleyHeight + 2) * 4, 0, 1)));
				...
				for (int y = 0; y < NibiruChunkProvider.CHUNK_SIZE_Y; y++)
				{
					if (y < NibiruChunkProvider.MID_HEIGHT + yDev)
					{
	...
	public void replaceBlocksForBiome(int par1, int par2, short[] arrayOfIDs, byte[] arrayOfMeta, BiomeGenBase[] par4ArrayOfBiomeGenBase)
	{
		...
		for (int var8 = 0; var8 < 16; ++var8)
		{
			for (int var9 = 0; var9 < 16; ++var9)
			{
				...
				for (int var16 = NibiruChunkProvider.CHUNK_SIZE_Y - 1; var16 >= 0; --var16)
				{
		...
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
		{
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
			{
				for (int x = 0; x < NibiruChunkProvider.CHUNK_SIZE_X; x++)
				{
					for (int z = 0; z < NibiruChunkProvider.CHUNK_SIZE_Z; z++)
					{
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen5.getNoise(cx * 16 + x, cz * 16 + z) / NibiruChunkProvider.CRATER_PROB)
						{
	...
	{
		return NibiruConfigManager.generateOtherMods ? "RandomLevelSource" : "NibiruLevelSource";
	}
	...
	public List getPossibleCreatures(EnumCreatureType par1EnumCreatureType, int i, int j, int k)
	{
		if (par1EnumCreatureType == EnumCreatureType.monster)
		{
			...
			monsters.add(new SpawnListEntry(NibiruEntityGiantWorm.class, 8, 2, 3));
			return monsters;
		}
		else
		{
