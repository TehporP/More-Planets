...
public class NibiruGenCaves extends GCCoreMapGenBaseMeta
{
							...
							for (int localX = l1; localX < i2; localX++)
							{
								...
								for (int localZ = l2; localZ < i3; localZ++)
								{
									...
									if (xfactorSq + zfactorSq < 1.0D)
									{
										...
										{
											if (blockIdArray[coords] == NibiruBlocks.nibiruBasicBlock.blockID)
											{
												if (metaArray[coords] == 6 || metaArray[coords] == 7)
												{
													blockIdArray[coords] = 0;
												}
												else if (metaArray[coords] == 5 && random.nextInt(NibiruGenCaves.BREAK_THROUGH_CHANCE) == 0)
												{
													blockIdArray[coords] = 0;
												}
											}
