...
public class NibiruRoomBoss extends GCCoreDungeonRoom
{
	public int sizeX;
	public int sizeY;
	public int sizeZ;
	...
	public NibiruRoomBoss(GCCoreMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
		...
		for (int i = this.posX - 1; i <= this.posX + this.sizeX; i++)
		{
			for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
			{
				for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
				{
					...
					{
						this.placeBlock(chunk, meta, i, j, k, cx, cz, NibiruBlocks.infectedNibiruZombieEgg.blockID, 0);
					}
					else
					{
	...
	protected void handleTileEntities(Random rand)
	{
		...
		this.worldObj.setBlock(this.spawnerCoords.posX, this.spawnerCoords.posY, this.spawnerCoords.posZ, NibiruBlocks.nibiruBasicBlock.blockID, 12, 3);
		...
		if (tile == null || !(tile instanceof NibiruTileEntityDungeonSpawner))
		{
			GCCoreTileEntityDungeonSpawner spawner = new NibiruTileEntityDungeonSpawner();
	...
	{
		return new NibiruRoomBoss(dungeon, x, y, z, dir);
	}
}