...
public class NibiruRoomSpawner extends GCCoreDungeonRoom
{
	int sizeX;
	int sizeY;
	int sizeZ;
	...
	public NibiruRoomSpawner(GCCoreMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
	...
	{
		return new NibiruRoomSpawner(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		for (final ChunkCoordinates spawnerCoords : this.spawners)
		{
			if (this.worldObj.getBlockId(spawnerCoords.posX, spawnerCoords.posY, spawnerCoords.posZ) == Block.mobSpawner.blockID)
			{
				...
				if (spawner != null)
				{
					spawner.getSpawnerLogic().setMobID(NibiruRoomSpawner.getMob(rand));
				}
			}
		}
	}
