...
public class PolongniusTileEntitySolar extends GCCoreTileEntityUniversalElectrical implements IMultiBlock, IPacketReceiver, IDisableableMachine, IInventory, ISidedInventory
{
	...
	public static final float MAX_GENERATE_WATTS = 20.0F;
	@NetworkedField(targetSide = Side.CLIENT)
	public float generateWatts = 0;
	private float ueMaxEnergy;

	public PolongniusTileEntitySolar()
	{
		this(0);
	}

	public PolongniusTileEntitySolar(float maxEnergy)
	{
	...
	public void updateEntity()
	{
			...
			if (!this.getDisabled(0) && this.ticks % 20 == 0)
			{
				...
				if (this.worldObj.isDaytime() && (this.worldObj.provider instanceof IGalacticraftWorldProvider || !this.worldObj.isRaining() && !this.worldObj.isThundering()))
				{
					...
					for (int x = -1; x <= 1; x++)
					{
						for (int z = -1; z <= 1; z++)
						{
							if (this.getBlockType() == PolongniusBlocks.polongniusSolarPanel)
							{
		...
		if (!this.worldObj.isRemote)
		{
			...
			{
				this.generateWatts = Math.min(Math.max(this.getGenerate(), 0), PolongniusTileEntitySolar.MAX_GENERATE_WATTS) / 20.0F;
			}
			else
			{
	...
	public float getGenerate()
	{
		...
		return 1 / 1000.0F * difference * difference * (this.solarStrength * (Math.abs(difference) * 1000.0F)) * this.getSolarBoost();
	}

	public float getSolarBoost()
	{
		return (float) (this.worldObj.provider instanceof ISolarLevel ? ((ISolarLevel) this.worldObj.provider).getSolarEnergyMultiplier() : 2.5F);
	}
		...
		for (int y = 1; y <= 2; y++)
		{
			for (int x = -1; x < 2; x++)
			{
				for (int z = -1; z < 2; z++)
				{
					...
					{
						FMLClientHandler.instance().getClient().effectRenderer.addBlockDestroyEffects(thisBlock.intX() + (y == 2 ? x : 0), thisBlock.intY() + y, thisBlock.intZ() + (y == 2 ? z : 0), PolongniusBlocks.polongniusSolarPanel.blockID & 4095, PolongniusBlocks.polongniusSolarPanel.blockID >> 12 & 255);
					}
	...
	public void readFromNBT(NBTTagCompound nbt)
	{
		...
		if (this.ueMaxEnergy > 2000.0F)
		{
			this.ueMaxEnergy /= 2000.0F;
		}
	...
	{
		int metadata = this.worldObj.getBlockId(this.xCoord, this.yCoord, this.zCoord);

		if (metadata >= PolongniusBlocks.polongniusSolarPanel.blockID)
		{
			metadata -= PolongniusBlocks.polongniusSolarPanel.blockID;
		}
	...
	{
		return StatCollector.translateToLocal(this.getBlockType() == PolongniusBlocks.polongniusSolarPanel ? "container.polongnius.solaradvanced.name" : "container.polongnius.solaradvanced.name");
	}
