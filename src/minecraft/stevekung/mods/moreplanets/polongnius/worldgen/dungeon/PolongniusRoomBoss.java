...
public class PolongniusRoomBoss extends GCCoreDungeonRoom
{
	public int sizeX;
	public int sizeY;
	public int sizeZ;
	...
	public PolongniusRoomBoss(GCCoreMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
		...
		for (int i = this.posX - 1; i <= this.posX + this.sizeX; i++)
		{
			for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
			{
				for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
				{
					...
					{
						this.placeBlock(chunk, meta, i, j, k, cx, cz, PolongniusBlocks.cheeseSlimeEgg.blockID, 0);
					}
					else
					{
	...
	{
		return new PolongniusRoomBoss(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		...
		this.worldObj.setBlock(this.spawnerCoords.posX, this.spawnerCoords.posY, this.spawnerCoords.posZ, PolongniusBlocks.polongniusBasicBlock.blockID, 15, 3);
		...
		if (tile == null || !(tile instanceof PolongniusTileEntityDungeonSpawner))
		{
			GCCoreTileEntityDungeonSpawner spawner = new PolongniusTileEntityDungeonSpawner();
