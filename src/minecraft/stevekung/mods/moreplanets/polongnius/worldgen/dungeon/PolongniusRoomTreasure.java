...
package stevekung.mods.moreplanets.polongnius.worldgen.dungeon;
...
public class PolongniusRoomTreasure extends GCCoreDungeonRoom
{
	int sizeX;
	int sizeY;
	int sizeZ;
	...
	public PolongniusRoomTreasure(GCCoreMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
	...
	public void generate(short[] chunk, byte[] meta, int cx, int cz)
	{
		...
		if (this.placeBlock(chunk, meta, hx, this.posY, hz, cx, cz, PolongniusBlocks.polongniusTreasureChest.blockID, 0))
		{
	...
	public ItemStack getGuaranteedLoot(Random rand)
	{
		...
		{
		case 0:
			return new ItemStack(PolongniusItems.cheeseOfMilkBlockCake, 1, 0);
		}
		return null;
	}
	...
	protected void handleTileEntities(Random rand)
	{
		for (final ChunkCoordinates chestCoords : this.chests)
		{
			...
			if (chest != null && chest instanceof PolongniusTileEntityTreasureChest)
			{
				ChestGenHooks info = ChestGenHooks.getInfo(ChestGenHooks.DUNGEON_CHEST);

				WeightedRandomChestContent.generateChestContents(rand, info.getItems(rand), (PolongniusTileEntityTreasureChest) chest, info.getCount(rand));

				((PolongniusTileEntityTreasureChest) chest).setInventorySlotContents(rand.nextInt(((PolongniusTileEntityTreasureChest) chest).getSizeInventory()), this.getGuaranteedLoot(rand));
			}
		}
	}
	...
	{
		return new PolongniusRoomTreasure(dungeon, x, y, z, dir);
	}
}